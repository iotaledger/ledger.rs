...
pub type Callback = fn(apdu_command: &ledger_transport::APDUCommand<Vec<u8>>, apdu_answer: &ledger_transport::APDUAnswer<Vec<u8>>);
...
impl TransportTCP {
    ...
    pub async fn exchange(&self, command: &APDUCommand<Vec<u8>>) -> Result<APDUAnswer<Vec<u8>>, LedgerTCPError> {
        ...
        let answer = APDUAnswer::from_answer(raw_answer).map_err(|_| LedgerTCPError::InnerError)?;
